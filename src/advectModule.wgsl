...
@group(0) @binding(3) var writeTex: texture_storage_2d<r32float, write>;
@group(0) @binding(4) var velocityTex: texture_storage_2d<rg32float, read>;

@compute @workgroup_size(8, 8) fn advect(
  ...
  if (all(id.xy >= vec2u(1)) && all(id.xy <= vec2u(uniforms.N))) {
    ...
    let samplePos = backPos / vec2f(textureDimensions(readTex).xy);
